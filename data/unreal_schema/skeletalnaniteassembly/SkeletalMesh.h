...
class USkeletalMesh : public USkinnedAsset, public IInterface_CollisionDataProvider, public IInterface_AssetUserData, public INodeMappingProviderInterface
{
	...
	ENGINE_API virtual ~USkeletalMesh() override;
...
private:
	...
	UPROPERTY(EditAnywhere, EditFixedSize, Category=LevelOfDetailModels)
...
public:
	 ...
	 * \param bAutoGenerated If set to true, the reduction settings are updated to point to a previous LOD with
	 *    editable data and the triangle reduction set to 50% for each LOD between the two (e.g. if this adds LOD 2,
	 *    and the first non-autogenerated LOD is 0, then the reduction settings will be at 25%).
	 * \return The newly created source model.
	 */
	 ...
	 * \param InLODIndex The potential LOD index to check for validity.
	 * \return \c true if the given LOD index in InLODIndex is valid.
	*/
...
protected:
#if WITH_EDITORONLY_DATA
	ENGINE_API virtual void UpdateMaterialUVChannelData(FMeshUVChannelInfo& UVChannelData, int32 MaterialIndex) override;
#endif
...
public:
	...
	{
		FProperty* ChangedProperty = FindFProperty<FProperty>(USkeletalMesh::StaticClass(), GET_MEMBER_NAME_CHECKED(USkeletalMesh, NaniteSettings));
...
#if WITH_EDITOR
	ENGINE_API virtual TArray<USkinnedAsset*> GetSkinnedAssetDependencies() const override;
	ENGINE_API virtual bool HasCompileDependencies() const override;
	ENGINE_API virtual bool HasAnyDependenciesCompiling() const override;
private:
	void OnNaniteAssemblyReferenceCached(USkeletalMesh* Reference);
#endif
...
#if WITH_EDITORONLY_DATA
	UPROPERTY(Transient)
	TArray<TObjectPtr<USkeletalMesh>> AssemblyReferenceCache;
#endif
...
public:
	...
	UE_DEPRECATED(5.7, "Please do not access this member directly, it will become private soon; use USkeletalMesh::GetClothingAssets_DEPRECATED().")
	UPROPERTY()
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return ClothingAssets_DEPRECATED;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	
	// The visual size of the bones in the viewport (saved between sessions). This is set from the viewport Character>Bones menu
	UE_DEPRECATED(5.7, "Please do not access this member directly, it will become private soon; use USkeletalMesh::GetBoneDrawSize() or USkeletalMesh::SetBoneDrawSize().")
	UPROPERTY()
	...
	void SetBoneDrawSize(float Size)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		BoneDrawSize = Size;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
protected:
...
#if WITH_EDITORONLY_DATA
	/** Array of user data stored with the asset */
	UE_DEPRECATED(5.7, "EditorOnly data will become private in the future. Use the GetAssetUserDataEditorOnly() accessor instead.")
	UPROPERTY(EditAnywhere, AdvancedDisplay, Instanced, Category=SkeletalMesh)
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return AssetUserDataEditorOnly;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	const TArray<TObjectPtr<UAssetUserData>>& GetAssetUserDataEditorOnly() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return AssetUserDataEditorOnly;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
public:
	 ...
	 */
	UE_DEPRECATED(5.7, "Use RemoveSourceModel() instead.")
	ENGINE_API void RemoveLODInfo(int32 Index);
	
	/*
	 * Reset whole entry
	 */
	UE_DEPRECATED(5.7, "Use SetNumSourceModels(0) instead.")
	...
	ENGINE_API virtual TArray<FSkeletalMeshLODInfo>& GetLODInfoArray() override;

	/*
	 * Returns whole array of LODInfo const. USkinnedAsset interface.
	 */
	UE_DEPRECATED(5.5, "Use GetLODInfo and GetLODNum instead.")
	ENGINE_API virtual const TArray<FSkeletalMeshLODInfo>& GetLODInfoArray() const override;
...
private:
	...
	/** Update the given LOD based on a previous LOD, setting it to auto-generated based on the
	 *  first non-autogenerated LOD, with 50% reduction in triangle count per-LOD.
	 */
	void CopyAutogeneratedLODInfoFromPreviousLOD(int32 InLODIndex);
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = Mesh, meta = (DisplayName = "Get All Skin Weight Profile Names", ScriptName = "GetAllSkinWeightProfileNames", Keywords = "skin weight profile"))
...
#if WITH_EDITORONLY_DATA
	UPROPERTY()
	TObjectPtr<USkeletalMeshEditorData> MeshEditorDataObject_DEPRECATED;

#endif //WITH_EDITORONLY_DATA
	
	UPROPERTY()
	TArray<FSkeletalMeshLODInfo> LODInfo_DEPRECATED;
