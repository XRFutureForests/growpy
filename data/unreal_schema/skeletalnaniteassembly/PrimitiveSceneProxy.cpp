...
,   bAnyMaterialUsesTemporalResponsiveness(false)
,   bAnyMaterialUsesMotionVectorWorldOffset(false)
,	bSupportsDistanceFieldRepresentation(false)
,	bSupportsHeightfieldRepresentation(false)
,	bLumenHeightfield(InProxyDesc.bLumenHeightfield)
,   bIsLandscapeProxy(false)
...
,	MinDrawDistance(InProxyDesc.MinDrawDistance)
{
	...
	bAnyMaterialUsesMotionVectorWorldOffset = false;
	{
		...
		{
			if (MaterialInterface && MaterialInterface->IsUsingWorldPositionOffset_Concurrent(ShaderPlatform))
			{
			...
			&& SupportsTSR(GetScene().GetShaderPlatform()))
		{
			...
			for (const UMaterialInterface* MaterialInterface : UsedMaterials)
			{
				if (MaterialInterface && MaterialInterface->IsUsingTemporalResponsiveness_Concurrent(ShaderPlatform))
				{
					bAnyMaterialUsesTemporalResponsiveness = true;
					break;
				}
			}
		}
		...
		for (const UMaterialInterface* MaterialInterface : UsedMaterials)
		{
			if (MaterialInterface && MaterialInterface->IsUsingMotionVectorWorldOffset_Concurrent(ShaderPlatform))
			{
				bAnyMaterialUsesMotionVectorWorldOffset = true;
				break;
			}
		}
	}
...
void FPrimitiveSceneProxy::BuildUniformShaderParameters(FPrimitiveUniformShaderParametersBuilder &Builder) const
{
...
#if RHI_RAYTRACING
	const bool bHasRayTracingRepresentation = IsRayTracingEnabled()
		? HasRayTracingRepresentation() && IsVisibleInRayTracing()
		: HasDistanceFieldRepresentation();
#else
	const bool bHasRayTracingRepresentation = HasDistanceFieldRepresentation();
#endif
	...
	Builder = FPrimitiveUniformShaderParametersBuilder{}
		.Defaults()
			...
			.HasRayTracingRepresentation(bHasRayTracingRepresentation)
			...
			.MaterialCacheDescriptor(GetMaterialCacheDescriptor())
			.IsFirstPerson(IsFirstPerson())
			.AnimationMinScreenSize(GetAnimationMinScreenSize());
	...
	if (IsNaniteMesh())
	{
		...
		Nanite::FResourcePrimitiveInfo NaniteResourcePrimitiveInfo = NaniteProxy->GetResourcePrimitiveInfo();
		...
		Builder.NaniteResourceID(NaniteResourcePrimitiveInfo.ResourceID)
			.NaniteHierarchyOffset(NaniteResourcePrimitiveInfo.HierarchyOffset)
			.NaniteAssemblyInfo(NaniteResourcePrimitiveInfo.AssemblyTransformOffset, NaniteResourcePrimitiveInfo.AssemblyTransformCount)
			.NaniteImposterIndex(NaniteResourcePrimitiveInfo.ImposterIndex)
...
{
	checkf(!bSupportsMaterialCache || MaterialCacheDescriptor != UINT32_MAX, TEXT("Primitive writes to material cache with an unallocated descriptor"));
	return MaterialCacheDescriptor;
}
...
ERayTracingProxyType FPrimitiveSceneProxy::GetRayTracingProxyType() const
{
	ERayTracingProxyType ProxyType = ERayTracingProxyType::None;
	if (bIsStaticMesh)
	{
		EnumAddFlags(ProxyType, ERayTracingProxyType::StaticMesh);
	}
	if (bIsHierarchicalInstancedStaticMesh)
	{
		EnumAddFlags(ProxyType, ERayTracingProxyType::HierarchicalInstancedStaticMesh);
	}
	if (bIsLandscapeGrass)
	{
		EnumAddFlags(ProxyType, ERayTracingProxyType::LandscapeGrass);
	}
	if (bSkinnedMesh)
	{
		EnumAddFlags(ProxyType, ERayTracingProxyType::SkeletalMesh);
	}
	if (bInstancedSkinnedMesh)
	{
		EnumAddFlags(ProxyType, ERayTracingProxyType::InstanceSkeletalMesh);
	}
	return ProxyType;
}
