...
def verify_skeleton_simple(usd_path: Path) -> dict:
    ...
    if not joints_match:
        ...
        if joint_names and not all(name.startswith("joint_") for name in joint_names):
            results["warnings"].append(
                "Some joints don't follow 'joint_N' naming convention"
            )

    # Check for bindTransforms
    if "uniform matrix4d[] bindTransforms" not in content:
        results["errors"].append(
            "Missing 'uniform matrix4d[] bindTransforms' attribute"
        )
    else:
    ...
    if "uniform matrix4d[] restTransforms" not in content:
        results["errors"].append(
            "Missing 'uniform matrix4d[] restTransforms' attribute"
        )
    else:
        results["info"]["has_rest_transforms"] = True

    # CRITICAL: Check for jointIndices (topology array) - MUST be uniform
    joint_indices_match = re.search(
        r"uniform int\[\] jointIndices = \[([^\]]+)\]", content
    )
    ...
    if "primvars:skel:jointIndices" in content:
        ...
        vertex_indices_match = re.search(
            r"primvars:skel:jointIndices = \[([^\]]+)\]", content
        )
        if vertex_indices_match:
            vertex_indices_str = vertex_indices_match.group(1)
            vertex_indices = [
                int(x.strip())
                for x in vertex_indices_str.split(",")[:20]
                if x.strip().lstrip("-").isdigit()
            ]
            if vertex_indices:
        ...
        results["warnings"].append(
            "No vertex skinning data found (primvars:skel:jointIndices)"
        )
...
def print_results(results: dict):
    ...
    if results["valid"]:
        ...
        print(
            "  3. Test skeletal deformation (rotate parent bone → children should move)"
        )
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python verify_skel_simple.py data/output/joint_indices_fix_test/Western_redcedar/Western_redcedar_tree_0000_tree_only_skeletal.usda"
        )
        sys.exit(1)
