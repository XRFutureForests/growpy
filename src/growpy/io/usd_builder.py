...
def _build_usdskel_from_bones(
    ...
    """
    from pxr import Sdf, UsdGeom, UsdSkel, Vt, Gf
    ...
    skel_root_prim.SetMetadata("apiSchemas", Sdf.TokenListOp.Create(prependedItems=["UnrealNaniteAssemblyRootAPI"]))
    skel_root_prim.CreateAttribute("unreal:naniteAssembly:meshType", Sdf.ValueTypeNames.Token).Set("skeletalMesh")
    ...
    skel_root_prim.CreateRelationship("unreal:naniteAssembly:skeleton").SetTargets([skel_path])
    ...
    # Bone format: (branch_id, bone_id, start_point, end_point, radius, mass, connected, parent_bone_id)
    for bone_idx, bone in enumerate(bones):
        ...
        if parent_idx == bone_idx or parent_idx < 0:  # Root bone
            joint_path = joint_name
        else:
            # Child bone - prefix with parent path
            parent_path = bone_to_joint_path.get(parent_idx, "")
            if parent_path:
        ...
        # Store world position for this bone
        ...
        if parent_idx == bone_idx or parent_idx < 0:  # Root bone
            # Root bone should be at origin (0,0,0) so mesh and skeleton align
            local_pos = Gf.Vec3d(0.0, 0.0, 0.0)
        else:
            # Child bone: position relative to parent
            parent_pos = bone_positions.get(parent_idx, Gf.Vec3d(0, 0, 0))
    ...
    Sdf.CopySpec(stage.GetRootLayer(), old_mesh_path, stage.GetRootLayer(), new_mesh_path)
