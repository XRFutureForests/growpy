...
def simplify_trunk_mesh(
    ...
    if len(trunk_faces) > CHUNK_FACE_LIMIT:
...
def _decimate_chunked(
    ...
    concatenated. Peak RAM stays proportional to chunk_limit instead of
    total face count. Boundary seams between chunks are negligible for
    Helios++'s spectra-based ray simulation, which does not require a
    watertight mesh -- do not "fix" this by merging chunks.
    ...
    logger.info(
        "Chunked decimation: %s faces -> %d chunks (~%s faces each, ratio %s)",
        f"{num_faces:,}", n_chunks, f"{chunk_limit:,}", ratio,
    )

    # Face centroids along Z axis for spatial splitting
    face_z = vertices[faces, 2].mean(axis=1)

    # Quantile-based split for roughly equal chunk sizes
    boundaries = np.percentile(face_z, np.linspace(0, 100, n_chunks + 1))
    ...
    for i in range(n_chunks):
        ...
        logger.debug(
            "  Chunk %d/%d: %d faces, %d verts (z=%.1f..%.1f)",
            i + 1, n_chunks, len(chunk_faces_local), len(chunk_verts), lo, hi,
        )
    ...
    logger.info(
        "Chunked decimation done: %s -> %s faces",
        f"{num_faces:,}", f"{len(result_faces):,}",
    )
