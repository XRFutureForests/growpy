...
def list_species() -> List[str]:
    ...
    grove_root = Path(__file__).parent.parent.parent / "the_grove_22"
...
def create_grove(species: Optional[str] = None, random_seed: Optional[int] = None) -> gc.Grove:
...
def add_tree_to_grove(grove: gc.Grove, position: Tuple[float, float, float], 
    ...
    pos = gc.Vector(*position)
    dir_vec = gc.Vector(*direction)
    grove.add_new_tree(pos, dir_vec, delay)


def simulate_grove(grove: gc.Grove, cycles: int) -> None:
    """Simulate grove growth for specified cycles."""
    grove.simulate(cycles)


def build_grove_models(grove: gc.Grove, build_options: Optional[Dict[str, Any]] = None) -> List:
    """Build 3D models from grove using Grove's core functionality."""
    if build_options is None:
        build_options = {}
    return grove.build_models(build_options)
...
def save_grove_to_json(grove: gc.Grove, output_path: Path) -> bool:
...
def save_model_to_usd(model, output_path: Path) -> bool:
