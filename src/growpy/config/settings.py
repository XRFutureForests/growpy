...
class GrowPyConfig:
    ...
    def get_data_directory(cls) -> Path:
        ...
        """
        current_file = Path(__file__)
        # Navigate from src/growpy/config/settings.py to the project root, then to data
        project_root = current_file.parents[
            3
        ]  # Go up 3 levels: config -> growpy -> src -> project_root
    ...
    def get_all_species(cls) -> List[str]:
        """
        Get list of all available species (alias for get_available_species).

        Returns:
            List of common names from the species lookup table.
        """
        return cls.get_available_species()
