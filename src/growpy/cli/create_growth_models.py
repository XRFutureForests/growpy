...
Yield table ingestion (--ingest-yield-tables) populates the local store from
external providers before calibration.  Use --list-providers to see available
sources.  Species without yield data are exported with uncalibrated Grove output.
"""
...
def _list_yield_providers():
    """Print all yield table providers and their availability status."""
    from pylometree.yield_tables import get_all_providers, get_available_providers
...
def _select_yield_providers(names):
    ...
    if names:
        ...
        for name in names:
            ...
            if not p.available():
                logger.warning(
                    "Provider %s not available: %s", name, p.status_message()
                )
                continue
            providers.append(p)
        return providers
...
def _ingest_yield_tables(
    config, script_dir: Path, *, clean: bool = False, provider_names=None
) -> int:
    ...
    providers = _select_yield_providers(provider_names)
    if not providers:
        logger.error(
            "No providers available. Install dependencies or use --list-providers."
        )
        return 0

    if clean and store_dir.exists():
        import shutil
    ...
    if manifest.entries:
        ...
        print()
        print(
            f"  Store contains {len(manifest.entries)} tables"
            f" for {len(species_set)} species"
        )
...
Note: Run prepare_assets.py first to copy species presets from Grove installation.
    ...
    parser.add_argument(
        ...
        "0 or unset = no target (default). Independent of [forest] max_height "
        "in forest.toml (the step 4 export height cap).",
    )
    parser.add_argument(
        "--seeds",
        type=int,
    ...
    parser.add_argument(
        "--ingest-yield-tables",
        action="store_true",
        help="Ingest yield tables from external providers before calibration.",
    )
    parser.add_argument(
        "--list-providers",
        action="store_true",
        help="List yield table providers and their status, then exit.",
    )
    parser.add_argument(
        "--clean-store",
        action="store_true",
        help="Clear existing yield table store before ingesting.",
    )
    parser.add_argument(
        "--providers",
        nargs="+",
    ...
    # Handle --list-providers (exit early)
    if args.list_providers:
        _list_yield_providers()
        return 0
    ...
    if args.ingest_yield_tables:
        _ingest_yield_tables(
            config,
            script_dir,
            clean=args.clean_store,
