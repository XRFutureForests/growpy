...
## First install the core
  ...
  1. Get **The Grove 2.3 Core** and unzip the package.
  2. Place the folder **the_grove_23** somewhere you can easily access it in the future – it provides a folder structure that is ready to be filled with twigs, bark textures, presets and add-ons. This is the suggested structure that will work great for most users.
...
The Blender add-on uses the core module, adds a user interface and a bunch of awesome tools for interactive simulation. The Blender add-on works in Blender version 4.2 LTS, 4.3 and 4.4.
  ...
  3. Open Blender, and add a new script directory in `Edit > Preferences > File Paths > Script Directories`. Point it to the location of **the_grove_23** like so:
...
To check your installation, you should see the tab `Grove 2.3` in the 3D view sidebar.
...
The Houdini add-on uses the core module and plugs it into Houdini’s procedural node network to grow and build trees that are ready for Houdini’s dynamics. Available with the Studio edition, the Houdini add-on works on Windows, Linux and macOS, in Houdini versions 19.5 and up, with Python versions 3.7 and up. And it will probably even work in future versions.
  ...
  4. In `$HOUDINI_USER_PREF_DIR`, find the **packages** folder or create it if it doesn’t exist yet (mind the lower case name). Copy the file `the_grove_23_in_houdini.json` from `the_grove_23/addons/the_grove_23_in_houdini/` to this **packages** folder.
  5. Edit this configuration JSON file and point `THEGROVE22` to the path to **the_grove_23**. If you use the default folder structure, it will find both the add-on and the core module. You’re good to go.
